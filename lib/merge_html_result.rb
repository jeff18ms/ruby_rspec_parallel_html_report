...
def examples_failures_of(report)
  ...
  if array.count < 3
    array << 0 #this is to account for some specs will not have pending tests, calcul_count_of expects array with a index of 3.
  end
  array
end
...
files = Dir.glob("tmp/**/report*.html")
#files = files_array
...
#File.open(ARGV[-1], 'w') {|f| f.write(report_name.to_html) }
File.open("tmp/All_Rspec_Test_Result.html", 'w') {|f| f.write(report_name.to_html) }
